...
inline std::wstring ReadRegistryString(const wchar_t* valueName, const wchar_t* fallback)
{
...
inline DWORD ReadRegistryDword(const wchar_t* valueName, DWORD fallback)
{
...
inline AppearanceSettings ReadAppearanceFromRegistry()
{
...
inline void WriteAppearanceToRegistry(const AppearanceSettings& settings)
{
	HKEY key = nullptr;
	DWORD disposition = 0;
...
inline bool ReadHookEnabledFromRegistry()
{
	DWORD enabled = 0;
...
inline void WriteHookEnabledToRegistry(bool enabled)
{
	HKEY key = nullptr;
	DWORD disposition = 0;
	LONG result = RegCreateKeyExW(
...
inline void UpdateSettingsWindowState(HWND hwnd, bool enabled)
{
...
inline void PopulateAppearanceControls(HWND hwnd, const AppearanceSettings& settings)
{
...
inline BOOL CALLBACK EnumMonitorProc(HMONITOR monitor, HDC, LPRECT, LPARAM lParam)
{
...
inline void PopulateMonitorList(HWND hwnd)
{
...
inline int CALLBACK EnumFontProc(const LOGFONTW* fontInfo, const TEXTMETRICW*, DWORD, LPARAM lParam)
{
...
inline void PopulateInstalledFonts(HWND hwnd)
{
...
inline bool ShowColorPalette(HWND hwnd, DWORD currentColor, DWORD* selectedColor)
{
...
inline std::wstring ReadControlText(HWND hwnd, int controlId)
{
	wchar_t buffer[LF_FACESIZE]{};
...
inline AppearanceSettings ReadAppearanceFromControls(HWND hwnd, const AppearanceSettings& current)
{
...
inline LRESULT CALLBACK SettingsWindowProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	if (msg == WM_NCCREATE)
	{
...
inline HWND CreateSettingsWindow(HINSTANCE instance, SettingsWindowContext* context)
{
	WNDCLASSEXW wc{};
