...
inline BOOL CALLBACK FindMonitorByIndexProc(HMONITOR monitor, HDC, LPRECT, LPARAM lParam)
{
...
inline RECT GetMonitorWorkAreaByIndex(DWORD monitorIndex)
{
...
inline LRESULT CALLBACK KeyPopupProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	if (msg == WM_NCCREATE)
	{
...
inline bool EnsurePopupClassRegistered(HINSTANCE instance)
{
	WNDCLASSEXW wc{};
...
inline HFONT CreatePopupFont(HWND owner, const std::wstring& fontName, DWORD fontSize)
{
	HDC hdc = GetDC(owner);
	if (hdc == nullptr)
	{
...
inline void ShowKeyPopup(HINSTANCE instance, const std::wstring& text, const AppearanceSettings& appearance)
{
	if (text.empty())
	{
		return;
